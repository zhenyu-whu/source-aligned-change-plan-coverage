...
1. 每个 evaluator 只读取 `cases.json`、`references/change-capability-framework-principles.md` 与 `references/phase-1-initial-change-plan.md`，不得读取 `oracle.json`。
2. 每个 case 由三名 fresh、互不共享输出的 evaluator 各判断一次。
3. Evaluator只返回推荐的 Change boundary、dependency/order、foundation/guard判断及理由，不得按 case ID 猜测测试意图。
