...
5. Phase 4: after `coverage-complete`, spawn a fresh writer subagent to generate source-window dossiers and semantic profiles from the stable global atom index and the original source documents. It writes `phase-works/phase-4/source-window-dossiers/` for every input change and input capability by copying the original source windows referenced by Phase 2/3 atoms, then writes `source-window-index.json`, `source-window-semantic-profile-review.md`, `source-window-grounding-issues.md`, `phase-4-agent-report.md`, and `trace/phase-4.trace.json`. The main agent then runs the validator, spawns an independent Phase 4 reviewer subagent, spawns an independent repair subagent if artifacts must change, reruns the validator, spawns a fresh independent reviewer again, and ends with:
