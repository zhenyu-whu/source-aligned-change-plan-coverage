...
- Phase 2 performs source-first atom extraction and Phase 2 aggregation. Each source document has one canonical extraction owner; Phase 2 raw atom files are immutable evidence after the phase completes.
...
3. Phase 2: create the lightweight work queue, spawn source-extraction writer subagents by source document or source-document batch, write one canonical `<source>.atoms.json` file per read-full source document, render each `<source>.atoms.md` mirror from JSON, run the fresh Phase 2 index/report writer subagent, and write `trace/phase-2.trace.json`. The main agent then refreshes `trace/manifest.json`, runs the validator, spawns an independent Phase 2 reviewer subagent, spawns an independent repair subagent if artifacts must change, reruns the validator after refreshing the manifest, and spawns a fresh independent reviewer again; after pass, freeze raw `.atoms.json` evidence and its rendered mirrors.
4. Phase 3: spawn a fresh writer subagent to normalize Phase 2 atoms into canonical `change-capability-anchors/obligation-atom-index.json`, audit source remainders, add precise missing atoms to the global index, split broad atoms, resolve duplicates/ambiguous ownership, assign normalized artifact projection, write all per-source coverage files and Phase 3 trace files under `phase-works/phase-3/`, write `phase-works/phase-3/phase-3-trace/source-to-global-atom-map.json`, write canonical `phase-works/phase-3/phase-3-trace/source-remainder-review.json` with mechanical uncovered source ranges and semantic review rows, render the JSON-backed Markdown mirrors, and write `trace/phase-3.trace.json`. The main agent then refreshes `trace/manifest.json`, runs the validator, spawns an independent Phase 3 reviewer subagent, spawns an independent repair subagent if artifacts must change, reruns the validator after refreshing the manifest, spawns a fresh independent reviewer again, and decides:
...
- Phase 2 outputs contain `phase-works/phase-2/source-obligation-atoms/work-queue.md`, `phase-works/phase-2/source-obligation-atoms/index.md`, one `phase-works/phase-2/source-obligation-atoms/<source>.atoms.md` and `.atoms.json` file for every source document listed as `read-full`, `trace/phase-2.trace.json`, and `phase-works/phase-2/phase-2-agent-report.md`.
...
- Phase 3 outputs contain `change-capability-anchors/obligation-atom-index.md`, `change-capability-anchors/obligation-atom-index.json`, `phase-works/phase-3/source-doc-manifest.md`, one `phase-works/phase-3/source-doc-coverage/<source>.coverage.md` file for every source document listed in the manifest, all `phase-works/phase-3/phase-3-trace/*.md` files, `phase-works/phase-3/phase-3-trace/source-to-global-atom-map.json`, `phase-works/phase-3/phase-3-trace/source-remainder-review.json`, `trace/phase-3.trace.json`, `phase-works/phase-3/coverage-review.md`, and `phase-works/phase-3/phase-3-agent-report.md`.
