...
- Phase 1 may include one zero-domain foundation candidate as a slicing and grounding aid. Phase 5 must not emit it as an executable final change; it renders a read-only foundation reference instead, and the executable roadmap starts with the first business change.
...
│   ├── foundation-runtime-substrate.md     # accepted/adjusted only; read-only design reference, not a change packet
...
└── phase-works/
    ...
    └── phase-5/
        ...
        ├── final-packet-index.json         # accepted/adjusted only; executable business changes only
...
- Phase 5: use a fresh independent subagent for source-window-grounded atom-driven change/capability plan refit, capability progression review, change complexity review, foundation reference generation, and executable business final packet generation.
...
6. Phase 5: after Phase 4 returns `grounded`, spawn a fresh writer subagent to refit the change/capability plan from the stable global atom index and Phase 4 source-window semantic profiles. It finalizes artifact projection for each direct atom, writes the current `phase-works/phase-5/` refit packet without any pass/iteration subdirectory, converts any foundation candidate into `foundation-reference/foundation-runtime-substrate.md` plus `.trace.json`, writes terminal mapping and executable business final packet artifacts when the status is `accepted` or `adjusted`, and always writes `trace/phase-5.trace.json`. It updates the latest effective root `change-plan.md` when needed, derives final `change-capability-anchors/<business-change-slug>/` packets for accepted/adjusted handoff, and explicitly carries every owner-scoped non-direct atom into the relevant final business change packet. The main agent then refreshes `trace/manifest.json`, runs the validator, spawns an independent Phase 5 reviewer subagent, spawns an independent repair subagent if artifacts must change, reruns the validator after refreshing the manifest, spawns a fresh independent reviewer again, and ends with:
...
- Phase 5 complexity, source-window grounding, capability-coupling, executable-roadmap, and foundation-reference gates pass according to `references/phase-5-targeted-plan-adjustment.md`, or Phase 5 records a blocker where that reference allows one.
