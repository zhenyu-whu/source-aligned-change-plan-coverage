...
Phase 2 is source-first obligation atom extraction plus a separate Phase 2 aggregation pass. Each source document is read once for canonical extraction, using the Phase 1 framework only as candidate ownership context. Phase 2 must not run one independent extraction pass per planned change. A source-backed atom may be assigned to a candidate change/capability, left unassigned, marked as candidate-new-change/capability, or classified as contextual/non-production/non-goal. After source extraction subagents finish, a fresh independent Phase 2 index/report subagent writes the Phase 2 aggregate `source-obligation-atoms/index.md` and `reports/phase-2-agent-report.md` from the raw extraction files. That aggregate is a Phase 2 inventory only, not Phase 3 global deduplication, coverage closure, or final ownership judgment. The Phase 2 source atom files are immutable raw extraction evidence after the phase completes.
