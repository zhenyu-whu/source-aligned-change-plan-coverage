...
- Phase 3's `change-capability-anchors/obligation-atom-index.json` is the canonical normalized global uniqueness and ownership registry; `change-capability-anchors/obligation-atom-index.md` is its renderer mirror.
...
2. Phase 1: if there is no current `change-plan.md`, spawn a fresh writer subagent to enumerate and read every source document, write `phase-works/phase-1/source-doc-manifest.md`, write `phase-works/phase-1/change-plan.md`, write `trace/phase-1.trace.json`, promote the latest effective plan to root `change-plan.md`, and generate the initial change/capability framework using the Phase 1 reference. The main agent then refreshes `trace/manifest.json`, runs the validator, spawns an independent Phase 1 reviewer subagent, spawns an independent repair subagent if artifacts must change, reruns the validator after refreshing the manifest, and spawns a fresh independent reviewer again before Phase 2.
