...
Phase 1 creates an initial change/capability framework from a full-source read. That framework is a slicing hypothesis, not the final authority. It may cite coarse source hints for human orientation, but it must not create obligation atoms, pending evidence inventories, line-range anchors, coverage statuses, or Phase 2 work queues. Phase 1 may propose at most one pre-business foundation candidate, and it must be a zero-domain engineering bootstrap; detailed Phase 1 slicing rules live in `references/phase-1-initial-change-plan.md`.

Phase 2 is source-first obligation atom extraction plus a separate Phase 2 aggregation pass and reviewer-facing visualization. Each source document is read once for canonical extraction, using the Phase 1 framework only as candidate ownership context. Phase 2 must not run one independent extraction pass per planned change. A source-backed atom may be assigned to a candidate change/capability, left unassigned, marked as candidate-new-change/capability, classified as contextual/non-production/non-goal, and assigned a candidate artifact projection. After source extraction subagents finish, a fresh independent Phase 2 index/report subagent writes the Phase 2 aggregate `phase-works/phase-2/source-obligation-atoms/index.md` and `phase-works/phase-2/phase-2-agent-report.md` from the raw extraction files. Then generate a deterministic static review app at `phase-works/phase-2/source-obligation-review/index.html` so human reviewers can inspect each source document's original line-numbered content beside obligation atom annotations, line ranges, and summaries. That aggregate and review app are Phase 2 inventory/review aids only, not Phase 3 global deduplication, coverage closure, final ownership judgment, or final projection judgment. The Phase 2 source atom files are immutable raw extraction evidence after the phase completes.

Phase 3 is coverage normalization and gap audit. It consumes the Phase 2 source atom files, reviews remaining source content semantically, adds source-backed missing atoms to the normalized global index when evidence is precise, splits broad atoms, resolves duplicates, assigns normalized artifact projection, and ensures every production-meaningful obligation has one normalized global atom, a justified non-direct/non-coverage status, or an explicit Phase 5 placement handoff. Phase 3 may use deterministic line-range helpers only to find mechanical candidates; semantic review is the gate.

Phase 4 is source-window dossier and semantic profile grounding. It uses Phase 2/3 atom line ranges as an index to copy reviewer-facing source-window dossiers for every input change and input capability. Those dossiers preserve original source lines, section context, related `GA-####` atoms, candidate ownership, and nearby contextual/remainder evidence so the Phase 5 subagent and human reviewer can inspect actual source semantics rather than only atom summaries. Phase 4 may identify refit pressure, grounding issues, and Phase 5 review signals, but it must not decide final change/capability ownership, rewrite the plan, or produce final packets.

Phase 5 is plan refit from the stable atom set and Phase 4 source-window semantic profiles. It evaluates the initial change order, capability progression, dependency graph, artifact projection, and change complexity from source-grounded semantic profiles rather than atom summaries alone. Phase 5 refit is not atom-count-based splitting: atom count is only a later complexity signal after source-window business/system outcomes, engineering dependency order, manual acceptance, foundation boundaries, and durable capability boundaries have been judged. Phase 5 may accept the Phase 1 framework or refactor changes/capabilities by splitting, merging, reordering, renaming, remapping atom ownership, or adjusting final artifact projection. Every refit decision must preserve atom-level traceability: each global atom is mapped from its input change/capability source dossier to its final change/capability, final artifact projection, contextual status, non-goal, or blocker. Phase 5 must preserve the distinction between implementation units and long-lived behavior boundaries; it must not refit the plan into a diagonal or same-name change/capability matrix merely to satisfy implementation complexity budgets.
...
Phase 2 source atom files are immutable raw extraction evidence. Phase 3's `change-capability-anchors/obligation-atom-index.md` is the normalized global uniqueness and ownership registry promoted to the proposal-facing root. `phase-works/phase-4/source-window-dossiers/` is a reviewer-facing copy of source windows selected by Phase 2/3 atom line ranges; it is not a new extraction pass and must not replace the original source documents, source atom ledgers, global atom index, or final change packets as source of truth. Phase 5 derives final change packets and capability views from the global index and Phase 4 source-window semantic profiles; it must not invent atoms without source evidence. `phase-works/phase-5/change-capability-human-plan.md` is a human-facing synthesis of the final change packets and capability progression; it must not replace the source-window dossiers, source atom ledgers, global atom index, or final change packets as source of truth.
...
3. Phase 2: create `phase-works/phase-2/source-obligation-atoms/work-queue.md` by lightly surveying the Phase 1 manifest, source roles, paths, document names, and line counts for batching only. Start with a semantic split, then merge compatible small/medium batches so the default extraction queue is no more than five batches unless the source set is genuinely large or context pressure justifies more. Then spawn source-extraction subagents partitioned by source document or source-document batch. Each subagent reads its assigned source document bodies in full, uses the Phase 1 plan only as candidate ownership context, extracts canonical source-backed atom candidates, records candidate artifact projection, records source remainder notes, and writes `phase-works/phase-2/source-obligation-atoms/<source>.atoms.md`. After extraction outputs exist, spawn a fresh Phase 2 index/report subagent to aggregate only Phase 2 raw extraction evidence and write `phase-works/phase-2/source-obligation-atoms/index.md` and `phase-works/phase-2/phase-2-agent-report.md`. After the aggregate exists, generate `phase-works/phase-2/source-obligation-review/index.html` as a deterministic static review app that shows the source document tree, each selected source document's original line-numbered content, and margin-style obligation atom annotations with covered line ranges and atom summaries.
...
- Final change packets, capability views, Phase 5 mapping, and root `change-plan.md` must preserve the same `GA-####` IDs from the global atom index.
...
- For every final capability, Capability Map `First change`, the first non-empty progression-matrix cell, the first roadmap `New` entry, the first direct owner in final change packets, the first occurrence in `change-capability-anchors/index.md`, capability views, and `phase-works/phase-5/change-capability-human-plan.md` must agree. No capability may be `Modified` before it is `New`.
...
- No final change exceeds the implementation-ready complexity budget unless Phase 5 records a specific indivisibility rationale, the rejected split options, and why the user must accept or decide the larger unit. A generic "one coherent loop" rationale is not sufficient.
- No final effective plan collapses into a capability-driven one-to-one roadmap. A mostly diagonal matrix, many single-change capabilities, or capability ids that paraphrase change slugs requires an explicit Phase 5 correction or a blocker with concrete source-backed exceptions.
...
- No final plan may contain a domain foundation/spine change before the first business workflow. If a foundation exists, it must be a zero-domain engineering bootstrap; the detailed banned scope is defined in the Phase 1 and Phase 5 reference files.
- No final effective plan contains multiple pre-business foundation/governance changes by default. If any standalone non-business system change remains after the first foundation, Phase 5 must record its independently runnable user/system or operational loop, failure path, evidence burden, and why it cannot be advanced inside the first business workflow that needs it.
